...
from data.datasets.data_utils import subsample_instances
...
class OxfordIIITPet(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any]:
        ...
        mask_path = path.replace('.jpg', '.npy').replace('images', 'data/masks')
        ...
        return (sample,mask_image), target, self.uq_idxs[idx]
...
def get_pets_datasets(train_transform, test_transform, train_classes=range(19), prop_train_labels=0.8,
    ...
    whole_training_set = OxfordIIITPet(root=args.dataset_dir, transform=train_transform, split='trainval', download=True)  # len = 3680
    ...
    test_dataset = OxfordIIITPet(root=args.dataset_dir, transform=test_transform, split='test', download=True)
