...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_path = os.path.join(self.mask_dir, self.data[idx].split('/')[-1].replace('.jpg', '.npy'))
        mask = np.load(mask_path)

        global_mean_values = np.array(image).mean(axis=(0, 1), keepdims=True)
        mask_image = np.where(np.repeat(mask[:, :, np.newaxis], 3, axis=2) == 0, global_mean_values, np.array(image))
        ...
        return (image, mask_image), target, idx
